...
CONTEXT_OVERRIDES = {
    '发表': {
        # 发表 + discourse/opinion/attitude complement → ABT (expressing ABOUT topic)
        'ABT_markers': [
            # Opinions/views
            '意见', '看法', '观点', '声明', '讲话', '评论', '见解', '主张',
            # Emotional stances (expressed ABOUT topic, not TO person)
            '不满', '遗憾', '关切', '支持', '反对', '抗议', '批评', '质疑',
            '感谢', '祝贺', '慰问', '哀悼', '谴责', '赞赏', '肯定', '否定',
        ],
        # 发表 + facial expression directed TO person → DA
        'DA_markers': ['微笑', '笑容', '善意'],
        'explanation': '发表 is ABT when expressing opinions/stances ABOUT something (意见/不满/批评), but DA when directing expressions TO someone (微笑).',
    },
    '表示': {
        'ABT_markers': ['关切', '遗憾', '不满', '欢迎', '支持', '反对', '感谢', '祝贺', '慰问', '谴责', '赞赏', '肯定', '否定', '抗议', '质疑'],
        'DA_markers': [],
        'explanation': '表示 is typically ABT when expressing stance ABOUT something.',
    },
    '提出': {
        'ABT_markers': ['建议', '意见', '看法', '观点', '批评', '质疑', '抗议', '要求', '问题', '方案'],
        'explanation': '提出 + discourse → ABT (putting forward discourse ABOUT topic).',
    },
    '有': {
        'MS_markers': ['兴趣', '信心', '把握', '好感', '印象', '了解', '感情', '感觉', '想法'],
        'EVAL_markers': ['益', '害', '利', '用', '效', '帮助', '作用', '影响'],
        'ABT_markers': ['意见', '看法', '研究', '评价'],
        'explanation': '有 varies by complement: 有+psychological→MS, 有+effect→EVAL, 有+opinion→ABT.',
    },
    '感兴趣': {
        'default': 'MS',
        'explanation': '感兴趣 is always MS - Y triggers interest (psychological state) in X.',
    },
    '不感兴趣': {
        'default': 'MS',
        'explanation': '不感兴趣 is always MS - Y fails to trigger interest in X.',
    },
    '有兴趣': {
        'default': 'MS',
        'explanation': '有兴趣 is always MS - X has interest (psychological state) toward Y.',
    },
    '没兴趣': {
        'default': 'MS',
        'explanation': '没兴趣 is always MS - X lacks interest (psychological state) toward Y.',
    },
    '没有兴趣': {
        'default': 'MS',
        'explanation': '没有兴趣 is always MS - X lacks interest (psychological state) toward Y.',
    },
}
...
class CorpusLookup:
    ...
    def analyse_in_context(self, predicate: str, complement: str = '', 
        ...
        if predicate in CONTEXT_OVERRIDES:
            ...
            # Check for default type (e.g., 感兴趣 is always MS)
            if 'default' in override:
                result['contextual_type'] = override['default']
                result['contextual_reason'] = override.get('explanation', f"'{predicate}' is typically this type")
                return result
            ...
            # Check DA markers
            if not result['contextual_type']:
                for marker in override.get('DA_markers', []):
                    if marker in context_text:
                        result['contextual_type'] = 'DA'
                        result['contextual_reason'] = f"Contains '{marker}' which indicates directed action"
                        result['learning_notes'].append(override.get('explanation', ''))
                        break
